...
# log_dir
log_dir ='./utterance3s/logs_gp_verifier_Phone2Phone/'
save_dir = './utterance3s/model_gp_verifier_Phone2Phone/'
...
Examples = collections.namedtuple("Examples", " images_L,images_R, label, utterance, count, steps_per_epoch")
Model = collections.namedtuple("Model", "pos_loss, neg_loss, distance")

def reject_outliers(data, m=2.0):
    data=np.asarray(data)

    return np.mean(data[np.absolute(data - np.mean(data)) < m * np.std(data)])
...
def read_labeled_image_list(image_list_file, img_dir):
    ...
    for line in f:
        # print line
        filename_L,filename_R, idx ,utterance_n= line[:-1].split(' ')
        filenames_L.append(img_dir + '/' + filename_L)
        filenames_R.append(img_dir + '/' + filename_R)
        clss.append(int(idx))
        utterance_ns.append(int(utterance_n))


    return filenames_L,filenames_R, clss,utterance_ns
...
def read_images_from_disk(input_queue):
    ...
    return example_L,example_R, label,utterance_n

def load_examples(datadir, lblfileaddress):
    filename = lblfileaddress
    imgdir = datadir
    # Reads pfathes of images together with their labels
    image_list_L,image_list_R,  label_class_list, u_list = read_labeled_image_list(filename, imgdir)
    # print image_list
    ...
    uttes = tf.convert_to_tensor(u_list, dtype=tf.int32)


    # Makes an input queue
    # input_queue = tf.train.slice_input_producer([images, labels],shuffle=isTraining)
    input_queue = tf.train.slice_input_producer([images_L,images_R, labels,uttes], shuffle=True)


    raw_image_L,raw_image_R, label,utte = read_images_from_disk(input_queue)
    ...
    image_batch_L,image_batch_R, label_batch,u_batch = tf.train.batch([input_images_L,input_images_R, label, utte],
...
def create_model(examples,isTraining):
    with tf.variable_scope("utils"):
        with tf.name_scope("left_tower"):
            with tf.variable_scope("tower_l"):


                pred_L  = create_tower_2mp_5c(examples.images_L,isTraining)
        # print pred.get_shape
        # print camconvA.get_shape()

        with tf.name_scope("right_tower"):
            with tf.variable_scope("tower_r"):

                pred_R = create_tower_2mp_5c(examples.images_R,isTraining)
        # print pred.get_shape
        # print camconvA.get_shape()
...
def main():
    ...
    if isTraining:
        lblfileaddress = "/media/sina/DATA/Speech/verification_Utternace/verification_sample_train"



        datadir = input_dir_train
    else:
        lblfileaddress = "/media/sina/DATA/Speech/verification_Utternace_Phone2Phone/verification_sample_test_utterance"
    ...
    with sv.managed_session() as sess:
        ...
        if isTraining:
            ...
            for i in range(max_steps):
                lbl,u_n, dist = sess.run([examples.label,examples.utterance, distance])
                t_lbl = np.append(t_lbl, lbl)
                # print lbl[0]
                t_dist = np.append(t_dist, dist)
                t_u=np.append(t_u,u_n)
                print (i, lbl[0],dist[0])
                # print i

            samples_distance={}
            u_label={}
            for i in range(t_lbl.size):
                if t_u[i] in dict.keys(samples_distance):
                    samples_distance[t_u[i]].append(t_dist[i])
                else:
                    samples_distance[t_u[i]]=[t_dist[i]]
                    u_label[t_u[i]]=[t_lbl[i]]

            u_dist=np.array([])
            u_lbl=np.array([])
            u_dist_median=np.array([])
            u_r05=np.array([])
            ...
            for k in dict.keys(samples_distance):
                SD=samples_distance[k]
                SD=np.asanyarray(SD)
                u_dist=np.append(u_dist,np.mean(SD))
                u_dist_median=np.append(u_dist_median,np.median(SD))


                u_r05 = np.append(u_r05, reject_outliers(SD, m=.5))

                u_r1 = np.append(u_r1, reject_outliers(SD, m=1))
                u_r2 = np.append(u_r2, reject_outliers(SD, m=2))
                u_r3 = np.append(u_r3, reject_outliers(SD, m=3))



                u_lbl=np.append(u_lbl,np.asanyarray(u_label[k]))
            ...
            EER, AUC = calculate_roc.calculate_eer_auc(t_lbl, t_dist, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_dist, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_dist_median, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_r1, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_r05, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_r2, plot=False)
            print EER, AUC

            EER, AUC = calculate_roc.calculate_eer_auc(u_lbl, u_r3, plot=False)
            print EER, AUC
